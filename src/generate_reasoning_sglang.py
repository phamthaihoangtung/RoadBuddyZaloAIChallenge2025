...
def main():
    ...
    for i in tqdm(range(0, len(test_data), args.batch_size), desc="Generating Reasoning"):
        ...
        for item in batch:
            ...
            _, video_inputs, _ = process_vision_info(messages, image_patch_size=processor.image_processor.patch_size)
            
            prompts.append(text)
            video_datas.append(video_inputs)

        outputs = llm.generate(prompt=prompts, video_data=video_datas, sampling_params=sampling_params)
